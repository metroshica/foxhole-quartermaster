...
import { Loader2, Factory, Trophy, Medal, Award } from "lucide-react";
    ...
    async function fetchLeaderboard() {
      ...
      try {
        const response = await fetch(
          `/api/leaderboard/production?period=${period}&limit=${limit}`
        );
        if (response.ok) {
        ...
        console.error("Failed to fetch production leaderboard:", error);
      } finally {
        setIsLoading(false);
      }
    }
      ...
      <CardContent>
        {isLoading ? (
          ...
          <div className="text-center py-8 text-muted-foreground">
            <p>No production activity yet</p>
            <p className="text-sm">Contribute to production orders to earn points!</p>
          </div>
        ) : (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {entry.contributionCount} contribution
                    {entry.contributionCount !== 1 ? "s" : ""}
                  </p>
                </div>
              ...
              <>
                ...
                <div className="flex items-center gap-3 p-2 rounded-md bg-primary/10 border border-primary/20">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {data.currentUserRank.contributionCount} contribution
                      {data.currentUserRank.contributionCount !== 1 ? "s" : ""}
                    </p>
                  </div>
                  <Badge variant="secondary" className="font-mono">
